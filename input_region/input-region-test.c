...
#define INPUT_WIDTH (WINDOW_WIDTH / 2)

struct input_region_client {
...
{
	static const char template[] = "/input-region-test-XXXXXX";
	const char *path;
	char *name;
	int fd;
	int ret;
...
static void
paint_buffer(struct input_region_client *client)
{
	...
	for (y = 0; y < WINDOW_HEIGHT; ++y) {
		for (x = 0; x < WINDOW_WIDTH; ++x) {
			if (x < INPUT_WIDTH)
				pixels[y * WINDOW_WIDTH + x] = 0xFFFF0000;
			else
				pixels[y * WINDOW_WIDTH + x] = 0x00000000;
		}
	}
}

static struct wl_buffer *
create_buffer(struct input_region_client *client)
{
...
{
	struct input_region_client *client = data;
...
static void
apply_input_region(struct input_region_client *client)
{
	client->input_region = wl_compositor_create_region(client->compositor);
	wl_region_add(client->input_region, 0, 0, INPUT_WIDTH, WINDOW_HEIGHT);
	wl_surface_set_input_region(client->surface, client->input_region);
}
...
xdg_surface_configure(void *data, struct xdg_surface *xdg_surface, uint32_t serial)
{
	...
	apply_input_region(client);
	wl_surface_attach(client->surface, client->buffer, 0, 0);
	wl_surface_damage_buffer(client->surface, 0, 0, WINDOW_WIDTH, WINDOW_HEIGHT);
	wl_surface_commit(client->surface);

	printf("Input region applied: left %dx%d receives input, right half is transparent\n",
			INPUT_WIDTH, WINDOW_HEIGHT);
}
...
{
	struct input_region_client client = {0};

	(void)argc;
	(void)argv;

	printf("Input region test: left half red and clickable, right half transparent\n");
	...
	if (!client.compositor || !client.shm || !client.xdg_wm_base) {
		fprintf(stderr, "Required globals missing (wl_compositor/wl_shm/xdg_wm_base)\n");
		goto cleanup;
	}
	...
	xdg_toplevel_set_title(client.xdg_toplevel, "Input Region Test");
	xdg_toplevel_set_app_id(client.xdg_toplevel, "input-region-test");
...
cleanup:
	...
	if (client.input_region)
		wl_region_destroy(client.input_region);
	if (client.buffer)
